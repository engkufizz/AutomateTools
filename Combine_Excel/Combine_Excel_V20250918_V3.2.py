...
def load_sources(
    ...
    # Concatenate with outer join to keep union of columns
    combined = pd.concat(frames, ignore_index=True, sort=True)

    # Reorder: metadata columns first if present
    cols = list(combined.columns)
    meta = [c for c in ["source_file", "source_sheet"] if c in cols]
    others = [c for c in cols if c not in meta]
    combined = combined[meta + others]
    return combined

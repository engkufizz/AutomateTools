...
class App:
    def __init__(self, root):
        ...
        self.outpath: Optional[str] = None  # full "save as" path picked by user (dir + filename)
        ...
        # Output save-as (single step: directory + filename)
        ttk.Label(frm, text="2) Choose output file name and location:").grid(row=2, column=0, sticky="w", pady=(10, 0))
        ttk.Button(frm, text="Save As...", command=self.choose_output_file).grid(row=2, column=1, sticky="e", pady=(10, 0))
        self.outpath_lbl = ttk.Label(frm, text="Output: not set")
        self.outpath_lbl.grid(row=3, column=0, columnspan=2, sticky="w")
        ...
        ttk.Label(fmt, text="Note: The chosen Save As name is used as base for all outputs.").grid(row=1, column=0, columnspan=2, sticky="w", pady=(5, 0))
    ...
    def choose_output_file(self):
        # Let user choose directory and type a filename in one step
        default_name = "Combined.xlsx" if self.out_xlsx.get() and not self.out_csv.get() else "Combined"
        initialfile = default_name if default_name.endswith(".xlsx") else f"{default_name}.xlsx"
        ...
        self.outpath_lbl.config(text=f"Output: {self.outpath}")
    ...
    def combine(self):
        ...
        if not (self.out_xlsx.get() or self.out_csv.get()):
            ...
            return
        if not self.outpath:
            messagebox.showwarning("No output file", "Please click 'Save As...' to choose output location and name.")
            return
        try:
            ...
            # Derive base name from chosen save-as path (ignore extension; we add timestamp + proper extension)
            base_name = os.path.splitext(os.path.basename(self.outpath))[0]
